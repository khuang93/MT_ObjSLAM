           ...
           //      cout<<"pix_d"<<pix_d<<endl;
                ...
                if (idx_d != -1) {
                    ...
                    if ((*it)->GetElement(i, MEMORYDEVICE_CPU) != 0) {
                        ...
                        single_obj_ITMView->depth->GetData(MEMORYDEVICE_CPU)[idx_d] = this->depth_Image->GetData(
                                MEMORYDEVICE_CPU)[idx_d];
...
#pragma omp parallel for
        ...
        for (int i = 0; i < this->depth_Image->dataSize; i++) {
            ...
            if (idx_d != -1) {
                ...
                if (is_background) {
                    bg_itmview->depth->GetData(MEMORYDEVICE_CPU)[idx_d] = this->depth_Image->GetData(
                            MEMORYDEVICE_CPU)[idx_d];
