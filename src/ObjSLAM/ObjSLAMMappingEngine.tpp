...
namespace ObjSLAM {
    ...
    void ObjSLAMMappingEngine<TVoxel, TIndex>::ProcessFrame() {
        ...
        if (view->GetObjVec().size() > 0) { //Verify that the view contains at least 1 object

            bool newObject = true; //reset the new object identifier
...
//#endif
            for (int t = 0; t < view->GetObjVec().size(); t++) { //Loop over all objects in the new view

                Object_View_Tup<TVoxel, TIndex> view_tuple = view->GetObjVec().at(t); //defined in ObjectView.h

                auto obj_inst_ptr = std::get<0>(view_tuple); //object intance pointer in the view
                auto label_ptr = obj_inst_ptr->GetClassLabel(); //label of the object instance

                int labelIndex = label_ptr->GetLabelIndex();

                //set the flag of background or not
                sceneIsBackground = labelIndex == 0 ? true : false;
//                cout<<"sceneIsBackground "<<sceneIsBackground<<endl; //Debug output

                std::shared_ptr<ITMLib::ITMView> itmview = std::get<1>(view_tuple); //the object itmview (rgbd image)

                auto &obj_ptr_vec_val = label_ptr->GetObjPtrVector(); //object pointer vector of the label
                auto obj_ptr_vec = &obj_ptr_vec_val;


                if (obj_ptr_vec->size() == 0) {
                    //If label contains no object, then this is guranteed a new object
                    newObject = true;
                } else {

                    typename std::vector<ObjSLAM::ObjectInstance_ptr<TVoxel, TIndex>>::iterator it;
                    for (it = obj_ptr_vec_val.begin(); it != obj_ptr_vec_val.end(); it++) { //loop over all existing objects in the vector
                        ObjectInstance_ptr<TVoxel, TIndex> existing_obj_ptr = *it;

                        if (obj_inst_ptr->CheckIsBackground()) {
                            //if backgound, then always false, because only 1 single background exists
                            newObject = false;
                        } else {
                            //Set the background flag to false
                            sceneIsBackground == false;
                            //Perform overlapping check in 2D and 3D
                            newObject = !(this->CheckIsSameObject2D(existing_obj_ptr, obj_inst_ptr) &&
                                          CheckIsSameObject3D(existing_obj_ptr, obj_inst_ptr));
//                            cout<<"sceneIsBackground "<<sceneIsBackground<<endl;
                        }
                        if (!newObject) {
                            //If detected as existing object, assign the pointer to the existing one, break out of the loop
                ...
                //After the whole loop if still not detected as existing object, then it is new object
                if (newObject) {
                    //Initializations
                    //Add the object to the vector of objects in the label
                    obj_inst_ptr->AddObjectInstanceToLabel();

                    //manage the object vector and active obejct vector
#pragma omp critical
                    {
                        if (obj_inst_ptr->CheckIsBackground()) {
                    ...
                    //Create a new scene for the new object
                    ...
                    //Create and set tracking states
...
//                    denseMapper->ResetScene(obj_inst_scene_ptr.get());
                ...
                } else { //if this is an existing object

                    //Update the camera pose stored in the object
                    auto tmp_t_state = obj_inst_ptr->GetTrackingState();
                    tmp_t_state->pose_d->SetFrom(this->t_state->pose_d);

                    //init object tracking with
                    if (do_Obj_tracking) {
                        ...
                        this->t_controller->Track(obj_inst_ptr->GetTrackingState().get(),
                                                  obj_inst_ptr->GetCurrentView().get());
...
#pragma omp parallel for private(sceneIsBackground)
        ...
        //Debug: write out point clouds as ply
//        write2PLYfile(this->renderState_RenderAll->raycastResult, "raycast_img" + to_string(imgNumber) + ".ply");
    ...
    void ObjSLAMMappingEngine<TVoxel, TIndex>::RefineTrackingResult() {
        ...
        int BG_weight = BG_object_ptr->GetScene()->index.getNumAllocatedVoxelBlocks() -
                        BG_object_ptr->GetScene()->localVBA.lastFreeBlockId;
...
//        obj_inst_ptr->UpdateVisibility();
    ...
    template<class TVoxel, class TIndex>
    void ObjSLAMMappingEngine<TVoxel, TIndex>::UpdateVisibilityAndViewCountOfObj(
            ObjectInstance_ptr <TVoxel, TIndex> obj_inst_ptr,
            const ORUtils::SE3Pose *pose) {
        sceneIsBackground = obj_inst_ptr->CheckIsBackground();
        auto *scene = obj_inst_ptr->GetScene().get();
    ...
    template<class TVoxel, class TIndex>
    void
    ObjSLAMMappingEngine<TVoxel, TIndex>::UpdateFarVisibilityOfObj(ObjectInstance_ptr <TVoxel, TIndex> obj_inst_ptr,
                                                                   const ORUtils::SE3Pose *pose) {
        sceneIsBackground = obj_inst_ptr->CheckIsBackground();
        auto *scene = obj_inst_ptr->GetScene().get();
        visualisationEngine->FindVisibleBlocks(scene,
...
#pragma omp parallel
            ...
            for (int idx = 0; idx < pcl->dataSize; idx++) {
                ...
                int index = findVoxel(voxelIndex,
                ...
                findPointNeighbors(p, sdf, Vector3i((int) ROUND(point.x), (int) ROUND(point.y), (int) ROUND(point.z)),
                                   voxelData, voxelIndex);
                for (int idx2 = 0; idx2 < 8; idx2++) {
                    index = findVoxel(voxelIndex,
                                      Vector3i((int) ROUND(p[idx2].x), (int) ROUND(p[idx2].y), (int) ROUND(p[idx2].z)),
                                      vmIndex, cache);
                    if (index != -1) {
                        voxelData[index] = TVoxel();
                    }
...
//  ITMLib::ITMLocalVBA<TVoxel> locVBA = scene->localVBA;
    ...
    template<class TVoxel, class TIndex>
    ObjUChar4Image *
    ObjSLAMMappingEngine<TVoxel, TIndex>::GetRGBImage(ObjectInstance_ptr <TVoxel, TIndex> obj_inst_ptr) {
    ...
    template<class TVoxel, class TIndex>
    ObjUChar4Image *
    ObjSLAMMappingEngine<TVoxel, TIndex>::GetImageFar(ObjectInstance_ptr <TVoxel, TIndex> obj_inst_ptr) {
...
//save stl
                if (saveSTL && (imgNumber / (reader_SkipFrames + 1)) % STL_Frequency == 0) {
                    ...
                    string stlname =
                            "Frame" + to_string(imgNumber) + "." + obj_inst_ptr->GetClassLabel()->GetLabelClassName() +
                            ".Object" + to_string(j) + ".stl";
